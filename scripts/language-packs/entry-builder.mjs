...
export const ENGLISH_FUNCTION_WORDS = new Set([
  ...
  "over", "under", "about", "between", "through", "during", "before", "after",
  "above", "below", "up", "down", "out", "off", "again",
  "not", "no", "yes", "here", "there", "then", "than", "too", "very", "just",
  "also", "only", "such", "some", "any", "each", "every", "all", "both", "few",
  "more", "most", "other", "another", "same", "so", "now",
  "and", "or", "but", "because", "if", "while", "although", "though", "when",
  "where", "why", "how", "as", "until", "unless", "since", "nor", "either",
  "neither", "whether",
]);
...
export function normalizeGloss(rawGloss) {
  let text = rawGloss.replace(/\([^)]*\)/g, " ").replace(/\s+/g, " ").trim();
  let pos = "n";
  if (/^to\s+/i.test(text)) {
    pos = "v";
    text = text.replace(/^to\s+/i, "");
  }
  text = text.replace(/^(a|an|the)\s+/i, "").trim().toLowerCase();
