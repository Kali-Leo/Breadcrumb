...
export function FeedbackEvidenceSection() {
  ...
  return (
    <section className="rounded border border-stone-200 bg-white p-3">
      <h3 className="font-semibold text-stone-600">{FEEDBACK_COPY.evidenceTitle}</h3>
      <p className="mt-1 text-stone-400">{FEEDBACK_COPY.evidenceHint}</p>
      {candidates.length === 0 ? (
                ...
                <div>
                  ...
                  <ul className="mt-1 space-y-0.5">
                    {evidence.claims.map((claim) => (
                      <li key={`${claim.occurredAtIso}-${claim.level}`} className="text-stone-500">
