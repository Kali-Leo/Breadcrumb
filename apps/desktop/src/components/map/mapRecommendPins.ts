 ...
 * every place that holds a visible recommendation, on every zoom level. One pin per place,
 * however many candidates it holds; markers counter-scale in the controller's tick so they
 * keep their on-screen size like the names do.
 ...
 * hole. Official asset used verbatim per the art discipline's "官方资产直用" rule; only
 * fill/stroke colors are ours (the map's amber accent). 24×24 viewBox, tip at
 * (12, ~21.5). */
const PIN_SVG =
