...
import { groundingSourceHeading } from "../../lib/grounding/sourceLabel";
...
export function GroundingMark({ sentence }: GroundingMarkProps) {
  ...
  const hover =
    quote === null ? label : `${label} — ${groundingSourceHeading(t, quote)} · ${quote.text}`;
      ...
      {open && (
        <span className="my-0.5 block border-stone-200 border-s-2 ps-2 text-xs text-stone-500">
          {quote === null ? (
            ...
            <>
              <span className="text-stone-400">{groundingSourceHeading(t, quote)} · </span>
              {quote.text}
            </>
          )}
        </span>
