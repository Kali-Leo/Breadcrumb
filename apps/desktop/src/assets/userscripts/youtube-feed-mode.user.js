...
(() => {
  ...
  // 也不发往这两个地方以外的任何地方。
  const IM_KEY_PORT = "yfm_im_port",
    IM_KEY_TOKEN = "yfm_im_token",
    IM_KEY_ON = "yfm_im_on";
  // 跨站点共用的暂存队列（脚本管理器的存储，不是这个站点的）：桌面版没开、或者用的是网页版时，
  // 记录先留在这里，等下一次能交出去的时候再交，一条不丢。
  const IM_PENDING = "yfm_pending";
    ...
    const settle = () => {
      ...
      if (watching && watching.watched >= 5) {
        ...
        imReport({
          ...
          t: watching.title,
          u: watching.up,
          pic: imCover(watching.id),
          dwell: Math.round(watching.watched),
...
输出 JSON：{"r":{"1":"p","2":"e",...}}，键为每行序号，值为 p/g/e/j/?，覆盖所有行，不要输出其他内容。`;
  ...
  if (localStorage.getItem("yfm_cache_v") !== "2") {
    // v2 专业模式加入情绪门槛：旧的 pro 按新标准重判
  ...
  function fmHash(bytes) {
    // murmurhash3_32 (x86, seed=0)，与训练侧 sklearn 一致
    const c1 = 0xcc9e2d51,
      c2 = 0x1b873593;
    let h = 0;
  ...
  const fmDeltaSave = () => {
    ...
    fmDeltaTimer = setTimeout(() => {
      try {
        localStorage.setItem("yfm_delta", JSON.stringify(fmDelta));
      } catch (e) {}
    }, 2000);
  };
  ...
  function applyTol() {
    ...
    ROUTE.low = tol === 0 ? -1 : Math.min(0.6, 0.1 + 0.008 * tol);
  ...
  tolUI.id = "yfm-tol";
  tolUI.title = ZH
    ? "分类容忍度：越低越多内容送大模型复核，更准但更费；越高越依赖本地小模型，几乎免费但可能有少量错分或漏内容。右侧数字为预计送大模型的比例。"
  ...
  async function flushBatch() {
    ...
    const clean = (s) =>
      String(s || "")
        ...
        .trim();
    const payload = items
      .map((it, i) => i + 1 + "|" + clean(it.title).slice(0, 80) + "|" + clean(it.owner))
      .join("\n");
    // 兼容模型偶发输出全称或数组格式
    const CODE = {
      p: "pro",
  ...
  function pump() {
    while (inflight < 32 && queue.length) {
      inflight++;
      queue
        .shift()()
        .finally(() => {
  ...
  const AD_SEL =
    "ytd-ad-slot-renderer, ytd-in-feed-ad-layout-renderer, ytd-display-ad-renderer, [class*=AdSlot]";
  function processCard(card) {
    if (card.dataset.yfmSeen) return;
    if (card.querySelector(AD_SEL)) {
      // 广告卡：标记后任何模式都显示
      card.dataset.yfm = "ad";
    ...
    queue.push(async () => {
      try {
        card.dataset.yfm = await classify(vid, title, owner);
      } catch (e) {
        card.dataset.yfm = "unk";
      } // 出错也要放行
    });
    pump();
  }
  ...
  function scan() {
    ...
    if (grid && !cont && mode !== "all" && backlog === 0) {
      if (!btn) {
        ...
        btn.style.cssText =
          "grid-column:1/-1;width:100%;padding:28px 0;margin:8px 0 40px;text-align:center;font-size:15px;color:#fff;background:#f00;border-radius:12px;cursor:pointer;";
  ...
  hint.style.cssText =
    "position:fixed;bottom:70px;left:24px;z-index:99998;display:none;padding:4px 12px;border-radius:999px;background:rgba(0,0,0,.7);color:#fff;font-size:12px;";
