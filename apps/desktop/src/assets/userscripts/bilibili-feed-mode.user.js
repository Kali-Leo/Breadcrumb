...
// @version      3.0.0
// @description  内置本地 AI 小模型 + 大模型复核，把B站首页推荐流分为「专业/精选娱乐/娱乐」，左下角开关一键切换。不填 API Key 也能用（本地模型离线分类）；填入 DeepSeek Key 后由大模型复核提升精度（用量实时显示，「容忍」滑条可控制用量，费用由你在 DeepSeek 后台自理）。本项目完全免费。不屏蔽任何广告与商业内容。可选把看过什么交给你自己的 Breadcrumb（新增网络权限仅用于此，不用这个功能则不发任何本机请求）。非官方工具，与哔哩哔哩无关联。
// @description:en  Filter your Bilibili home feed into Learn / Feel-good / Fun with one click. A built-in local AI model works offline; add a DeepSeek key for cloud review and higher accuracy, with live usage display and a tolerance slider. Does not block ads. Optionally hands what you watch to your own copy of Breadcrumb; the network permission is used for this alone, and without it no local requests are made. Unofficial tool, not affiliated with Bilibili.
...
(() => {
  ...
  // ================= 交给 Breadcrumb（默认关闭） =================
  // 只有通过开关条的 🔗 打开之后，下面这些才会做事；没打开时全部空转，脚本行为与不带此功能时一致。
  // 目标是你自己的 Breadcrumb：桌面版在本机 127.0.0.1 上收，网页版由你打开的 Breadcrumb 页面收。
  // 发的是标题、作者名、视频 id、封面地址、观看时长；不发 Cookie、不发账号、不发任何身份信息，
  // 也不发往这两个地方以外的任何地方。
  const IM_KEY_PORT = "bfm_im_port",
    IM_KEY_TOKEN = "bfm_im_token",
    IM_KEY_ON = "bfm_im_on";
  // 跨站点共用的暂存队列（脚本管理器的存储，不是这个站点的）：桌面版没开、或者用的是网页版时，
  // 记录先留在这里，等下一次能交出去的时候再交，一条不丢。
  const IM_PENDING = "bfm_pending";
  const IM_MAX_PENDING = 2000;
  const IM_BATCH = 200;
  const IM_CHANNEL = "breadcrumb.browsing/1";
  let IM_PORT = (localStorage.getItem(IM_KEY_PORT) || "").trim();
  let IM_TOKEN = (localStorage.getItem(IM_KEY_TOKEN) || "").trim();
  const imOn = () => localStorage.getItem(IM_KEY_ON) === "1";
  const imUrl = (path) => "http://127.0.0.1:" + IM_PORT + path;
  ...
  function imPost(url, headers, body) {
    if (typeof GM_xmlhttpRequest !== "function") {
      return fetch(url, { method: "POST", headers, body, keepalive: true }).then((r) =>
        r.text().then((text) => ({ ok: r.ok, status: r.status, text })),
      );
    }
  ...
  // 暂存队列的读写。脚本管理器的存储是跨站点的，所以 B 站上攒下的东西，
  // 打开 Breadcrumb 网页时同一个脚本能拿到。
  function imPending() {
    try {
      const raw =
        typeof GM_getValue === "function"
          ? GM_getValue(IM_PENDING, "[]")
          : localStorage.getItem(IM_PENDING) || "[]";
  ...
  function imSavePending(list) {
    try {
      const text = JSON.stringify(list.slice(-IM_MAX_PENDING));
      if (typeof GM_setValue === "function") GM_setValue(IM_PENDING, text);
      else localStorage.setItem(IM_PENDING, text);
    } catch (e) {
      /* 存不下就存不下，下一条照常记 */
    }
  }
  const imStore = (batch) => imSavePending(imPending().concat(batch));
  const imDrop = (count) => imSavePending(imPending().slice(count));
  // 配对：把 Breadcrumb「发现」页上的那串字粘进来，换一把只属于这个浏览器的钥匙。
  // 那串字只能用一次，用过之后 Breadcrumb 会换新的。
  function imPairWithCode(raw) {
    const parts = String(raw)
      .trim()
      .match(/^(\d{2,5})[-: ]([0-9a-f]{16,64})$/i);
    if (!parts) {
      alert(
        ZH
          ? "这串字看着不对。回到 Breadcrumb 的「发现」页，再复制一次。"
          : "That does not look right. Copy the line from Breadcrumb's Discover page again.",
      );
      return;
    }
    const port = parts[1],
    ...
    imPost(
      ...
      .then((d) => {
        if (!d || !d.token) {
          alert(
            ZH
              ? "没连上。回到 Breadcrumb 的「发现」页复制新的一串再试一次。"
              : "That did not connect. Copy a fresh line from Breadcrumb's Discover page and try again.",
          );
          return;
        }
        localStorage.setItem(IM_KEY_PORT, port);
        localStorage.setItem(IM_KEY_TOKEN, d.token);
        localStorage.setItem(IM_KEY_ON, "1");
        IM_PORT = port;
        IM_TOKEN = d.token;
        ...
        alert(
          ZH
            ? "连上了。照常刷就行，看过的东西会出现在 Breadcrumb 的「发现」页上。"
            : "Connected. Just browse as usual; what you watch appears on Breadcrumb's Discover page.",
        );
      })
      .catch(() =>
        alert(
          ZH
            ? "没连上。确认 Breadcrumb 正开着，再试一次。"
            : "That did not connect. Make sure Breadcrumb is open and try again.",
        ),
      );
  }
  // 攒下的东西补送给桌面版，一批一批来，送成一批就从暂存里划掉一批。
  function imResend() {
    if (!IM_PORT || !IM_TOKEN) return;
    const batch = imPending().slice(0, IM_BATCH);
    if (!batch.length) return;
    imPost(
      imUrl("/events"),
      { "Content-Type": "application/json", "X-Breadcrumb-Token": IM_TOKEN },
      JSON.stringify(batch),
    )
      .then((r) => {
        if (!r.ok) return;
        imDrop(batch.length);
        setTimeout(imResend, 500);
      })
      .catch(() => {
        /* 下次再说 */
      });
  }
  // 交给 Breadcrumb 网页：只往本页自己发消息，页面确认收下之后才从暂存里划掉。
  function imHandOff() {
    let sending = null;
    const send = () => {
      if (sending) return;
      const pending = imPending();
      if (!pending.length) return;
      const count = Math.min(pending.length, IM_BATCH);
      sending = { batch: String(Date.now()) + "." + Math.random().toString(36).slice(2), count };
      window.postMessage(
        {
          channel: IM_CHANNEL,
          kind: "events",
          batch: sending.batch,
          events: pending.slice(0, count),
        },
        location.origin,
      );
    };
    window.addEventListener("message", (e) => {
      ...
      const d = e.data;
      if (!d || typeof d !== "object" || d.channel !== IM_CHANNEL) return;
      if (d.kind === "ready") {
        send();
        return;
      }
      if (d.kind === "stored" && sending && d.batch === sending.batch) {
        imDrop(sending.count);
        sending = null;
        send();
      }
    });
  ...
  // Breadcrumb 网页版打开着的时候，这个脚本在那一页上要做的事只有交接，别的什么都不做。
  const IM_BREADCRUMB_PAGE =
    (location.hostname === "kali-leo.github.io" &&
      location.pathname.indexOf("/Breadcrumb/") === 0) ||
    ((location.hostname === "localhost" || location.hostname === "127.0.0.1") &&
      ["1420", "4173", "5173"].indexOf(location.port) >= 0);
  if (IM_BREADCRUMB_PAGE) {
    imHandOff();
    return;
  }
  const imQueue = [];
  ...
  function imFlush() {
    imTimer = null;
    if (!imOn() || !imQueue.length) return;
    const batch = imQueue.splice(0, IM_BATCH);
    // 没配对过（用的是网页版，或者还没连桌面版）：直接进暂存，等下次交出去。
    if (!IM_PORT || !IM_TOKEN) {
      imStore(batch);
      imSetState("queued");
      return;
    ...
    imPost(
      ...
      .then((r) => {
        ...
        // 403 = 这把钥匙不好使了：记录仍然留着，等重新连上再补送，一条都不丢
        if (r.status === 403) {
          imStore(batch);
          imSetState("bad");
          return;
      ...
      .catch(() => {
        // Breadcrumb 没开着：进暂存，下次再试，不丢数据
        imStore(batch);
        imFails++;
        imSetState("offline");
        if (!imTimer)
          imTimer = setTimeout(imFlush, Math.min(60000, 5000 * 2 ** Math.min(imFails, 4)));
      });
  }
  ...
  if (/^\/video\//.test(location.pathname)) {
    ...
    const send = () => {
      ...
      imReport({
        ...
        id: bvid,
        t: (
          (h1 && (h1.getAttribute("title") || h1.textContent)) ||
          document.title.replace(/_哔哩哔哩.*$/, "")
        ).trim(),
        u: ((up && up.textContent) || "").trim(),
        pic: "",
        dwell: Math.round(watched),
  ...
  document.addEventListener(
    "click",
    (e) => {
      if (!imOn() || !e.target.closest) return;
  ...
  const API_URL =
    localStorage.getItem("bfm_api_url") || "https://api.deepseek.com/chat/completions";
...
输出 JSON：{"r":{"1":"p","2":"e",...}}，键为每行序号，值为 p/g/e/j/?，覆盖所有行，不要输出其他内容。`;
  ...
  if (localStorage.getItem("bfm_cache_v") !== "2") {
    // v2 引入精选娱乐：旧的 ent 里可能藏着 good，清掉重判
    for (const k of Object.keys(cache)) if (cache[k] === "ent") delete cache[k];
    localStorage.setItem("bfm_cache_v", "2");
  }
  if (localStorage.getItem("bfm_cache_v") !== "3") {
    // v3 专业模式加入情绪门槛：旧的 pro 按新标准重判
  ...
  if (Object.keys(cache).length > 5000) {
    for (const k of Object.keys(cache).slice(0, 2500)) delete cache[k];
  }
  let saveTimer = null;
  const save = () => {
  ...
  const fmDeltaSave = () => {
    clearTimeout(fmDeltaTimer);
    fmDeltaTimer = setTimeout(() => {
      try {
        localStorage.setItem("bfm_delta", JSON.stringify(fmDelta));
      } catch (e) {}
    }, 2000);
  };
  ...
  function fmLearn(title, up, y) {
    // y: 1=专业 0=非专业（仅接受 LLM 判定，防自我强化）
    const { counts, norm } = fmFeatures(title, up);
  ...
  function applyTol() {
    ...
    ROUTE.low = tol === 0 ? -1 : Math.min(0.6, 0.1 + 0.008 * tol); // 低于此：本地判非专业，不送 LLM
  ...
  const IM_STATE_TEXT = {
    ...
    offline: [
      ...
      "Breadcrumb: not reachable, what you watched is being kept",
    ],
    queued: [
      "Breadcrumb：记录存着，打开 Breadcrumb 时交过去",
      "Breadcrumb: keeping what you watched, handed over when you open Breadcrumb",
    ],
  };
  ...
  imBtn.onclick = () => {
    if (!imOn()) {
      // 没开始记录：先说清楚这是什么、数据去哪，再问怎么连。
      if (
        !confirm(
          ZH
            ? "把你在这里看过和点开的视频（标题、作者、封面、看了多久）交给你自己的 Breadcrumb，" +
                "在那边整理成只有你能看的兴趣页面。数据只留在你的设备上，不发给任何网站。\n\n开始记录？"
            : "Hand what you watch and open here (title, author, cover, how long you watched) to your own " +
                "Breadcrumb, where it becomes an interest page only you can see. It stays on your device and " +
                "is not sent to any website.\n\nStart recording?",
        )
      )
        return;
      const code = (
        prompt(
          ZH
            ? "电脑上装了 Breadcrumb 的话，把它「发现」页上的那串字粘在这里。\n用网页版的话，留空点确定就行。"
            : "If Breadcrumb is installed on this computer, paste the line from its Discover page here.\n" +
                "Using the web version? Leave this empty and press OK.",
          "",
        ) || ""
      ).trim();
      localStorage.setItem(IM_KEY_ON, "1");
      if (!code) {
        ...
        alert(
          ZH
            ? "好了。你打开 Breadcrumb 网页的时候，记录会自动交过去。"
            : "Done. What you watch is handed over whenever you open Breadcrumb on the web.",
        );
        return;
      }
      imPairWithCode(code);
    ...
    // 已经在记录：确定＝继续；取消＝停下来（再确认一次，防误触）
    if (
      confirm(
        ZH
          ? "正在记录。\n\n「确定」继续；「取消」停下来。"
          : "Recording.\n\nOK keeps it going; Cancel stops it.",
      )
    )
      return;
    if (
      !confirm(
        ZH
          ? "要停止记录吗？已经存着还没交出去的记录会一起清掉。"
          : "Stop recording? Anything kept but not yet handed over is cleared too.",
      )
    )
      return;
    localStorage.removeItem(IM_KEY_ON);
    localStorage.removeItem(IM_KEY_TOKEN);
    localStorage.removeItem(IM_KEY_PORT);
    IM_TOKEN = "";
    IM_PORT = "";
    imSavePending([]);
    imSetState("off");
    alert(ZH ? "已经停下来了，刷新页面生效。" : "Stopped. Reload to apply.");
  };
  ...
  function addUsage(u) {
    ...
    tok.in += u.prompt_tokens || 0;
    tok.out += out;
    tok.hit += hit;
    tok.req++;
    tok.c = (tok.c || 0) + cost;
    tok.dIn += u.prompt_tokens || 0;
    tok.dOut += out;
    tok.dHit += hit;
  ...
  function biliFetch(url) {
    const run = async () => {
      ...
      if (RATE_CODES.has(d.code)) {
        ...
        biliBackoffUntil = Date.now() + 60000 * 2 ** (biliBackoffLevel - 1);
        stats.biliLimited = (stats.biliLimited || 0) + 1;
        stats.lastLimitCode = d.code;
        console.warn(
          "[bfm] B站接口限流(code " + d.code + ")，退避 " + 2 ** (biliBackoffLevel - 1) + " 分钟",
        );
  ...
  function kwClassify(title, owner, tags) {
    ...
    const score = (kws) => {
      let s = 0;
      for (const kw of kws) {
        if (tagText.includes(kw)) s += 2;
        if (titleText.includes(kw)) s += 1;
      }
      return s;
    };
    const j = score(JUNK_KW),
      p = score(PRO_KW),
      e = score(ENT_KW);
  ...
  // ---------- LLM 批量分类 ----------
  const batch = []; // {bvid, title, owner, tags, resolve}
  let batchTimer = null;
  ...
  let llmFailStreak = 0,
    llmCooldownUntil = 0; // 连续失败熔断：60秒内不再发请求，直接走关键词兜底
  async function flushBatch() {
    ...
    if (Date.now() < llmCooldownUntil) {
      for (const it of items) it.resolve(null);
      return;
    }
    ...
    const payload = items
      .map(
        (it, i) =>
          ...
          1 +
          "|" +
          clean(it.title).slice(0, 80) +
          "|" +
          clean(it.owner) +
          "|" +
          (it.tags || []).map(clean).join(","),
      )
      .join("\n");
    // 兼容模型偶发输出全称或数组格式
    const CODE = {
    ...
    for (let attempt = 0; attempt < 2 && !map; attempt++) {
      ...
      try {
        ...
        for (const [idx, v] of entries) {
          ...
          if (Number.isInteger(idx) && idx >= 0 && idx < items.length && c)
            map[items[idx].bvid] = c;
        }
      } catch (e) {
  ...
  async function classify(id, title, owner, presetTags) {
    ...
    if (!API_KEY || ROUTE.off) {
      ...
      if (lp >= 0.7) return "pro";
      if (lp > 0.3) return "unk"; // 置信度不足：丢弃（过滤模式下不显示），供给无限不心疼
    ...
    if (mode === "pro" || mode === "all") {
      if (lp < ROUTE.low && Math.random() >= ROUTE.audit) {
        stats.localSkip = (stats.localSkip || 0) + 1;
        return "ent";
      }
      if (lp >= ROUTE.high) {
        stats.localPass = (stats.localPass || 0) + 1;
        return "pro";
      }
    }
  ...
  // ---------- 卡片处理 ----------
  // 某分类在某模式下是否该显示
  const matchMode = (cls, m) =>
    m === "gent" ? cls === "good" : m === "ent" ? cls === "ent" || cls === "good" : cls === m;
  let inflight = 0;
  const queue = [];
  // 并发上限必须 ≥ LLM 批量大小：每个任务都在等同一次批量请求返回，上限过低会把实际批量压小、吞吐骤降。
  // 对B站接口的保护由 biliFetch 全局串行节流承担，这里放开无风险
  function pump() {
    while (inflight < 48 && queue.length) {
      inflight++;
      queue
        .shift()()
        .finally(() => {
  ...
  function processCard(card) {
    ...
    card.dataset.bfmSeen = "1";
    if (shownBvids.has(bvid)) {
      // 缓冲池已经注入过这个视频，原生卡片按重复隐藏
    ...
    queue.push(async () => {
      try {
        setCls(card, await classify(bvid, title, owner), bvid, owner);
      } catch (e) {
        setCls(card, "unk", bvid, owner);
      } // 出错也要放行，不能永远隐藏
    });
    pump();
  }
  ...
  function processSpecial(el) {
    ...
    if (liveId) {
      ...
      queue.push(async () => {
        try {
          setCls(
            el,
            await classify("live:" + liveId, "【直播】" + title, owner, []),
            "live:" + liveId,
            owner,
          );
        } catch (e) {
          setCls(el, "unk", "live:" + liveId, owner);
        }
      });
      pump();
      return;
    ...
    const label =
      (el.querySelector(".floor-title") || {}).innerText || el.innerText.trim().split("\n")[0];
  ...
  const poolCountFor = (m) =>
    pool.filter((p) => matchMode(p.cls, m) && Date.now() - p.ts < POOL_TTL_MS).length;

  async function fetchFeedPage() {
    stats.feedPages++;
    freshIdx++;
    const u =
      "https://api.bilibili.com/x/web-interface/wbi/index/top/feed/rcmd?ps=30&fresh_type=4&fresh_idx=" +
      freshIdx +
      "&fresh_idx_1h=" +
      freshIdx +
      "&brush=" +
  ...
  async function ensurePool(force) {
    ...
    if (poolCountFor(mode) < POOL_MIN)
      poolCooldownUntil = Math.max(Date.now() + 30000, biliBackoffUntil);
    fetching = false;
  }
  ...
  tolUI.id = "bfm-tol";
  tolUI.title = ZH
    ? "分类容忍度：越低 → 越多内容送大模型复核，更准但更费；越高 → 越依赖本地小模型，几乎免费但可能出现少量错分或漏内容。右侧数字为预计送大模型的比例。"
    : "Tolerance: lower sends more items to the cloud model (more accurate, costs more); higher relies on the free local model (a few misclassifications possible). The number shows the estimated share sent to the cloud.";
  tolUI.innerHTML =
    "<span>" +
    (ZH ? "容忍" : "Tol") +
    '</span><input type="range" min="0" max="100" step="10"><b></b>';
  if (API_KEY) sw.appendChild(tolUI);
  const tolInput = tolUI.querySelector("input"),
    tolLabel = tolUI.querySelector("b");
  const tolRender = () => {
    tolLabel.textContent = "AI" + Math.round(fmShareEst() * 100) + "%";
  };
  tolInput.value = tol;
  tolRender();
  tolInput.oninput = () => {
  ...
  function captureTemplate() {
    ...
    for (const fc of document.querySelectorAll(".feed-card")) {
      if (
        fc.querySelector('a[href*="video/BV"]') &&
        fc.querySelector("img") &&
        fc.querySelector(".bili-video-card__info--tit")
      ) {
        ...
        template.querySelectorAll("[data-bfm], [data-bfm-seen]").forEach((e) => {
          delete e.dataset.bfm;
          delete e.dataset.bfmSeen;
        });
  ...
  function buildCard(p) {
    ...
    el.querySelectorAll("a").forEach((a) => {
      a.setAttribute("href", "https://www.bilibili.com/video/" + it.bvid + "/");
    ...
    if (tit) {
      tit.setAttribute("title", it.title);
      (tit.querySelector("a") || tit).textContent = it.title;
    }
    ...
    if (date && it.pubdate) {
      ...
      date.textContent = " · " + (d.getMonth() + 1) + "-" + String(d.getDate()).padStart(2, "0");
    }
    const sv = el.querySelectorAll(".bili-video-card__stats--item");
    if (sv[0] && it.stat)
      (sv[0].querySelector("span:last-child") || sv[0]).textContent = fmtNum(it.stat.view);
    if (sv[1] && it.stat)
      (sv[1].querySelector("span:last-child") || sv[1]).textContent = fmtNum(it.stat.danmaku);
  ...
  function injectFromPool(n) {
    ...
    for (let i = pool.length - 1; i >= 0; i--)
      if (now - pool[i].ts > POOL_TTL_MS) pool.splice(i, 1);
    const pickOne = (arr) => {
      for (let i = 0; i < arr.length; i++) {
        if (matchMode(arr[i].cls, mode) && !shownBvids.has(arr[i].item.bvid))
  ...
  function maybeInject() {
    ...
    const visInjected = cont
      ? [...cont.querySelectorAll("[data-bfm-injected]")].filter((e) => e.offsetHeight > 0).length
      : 0;
  ...
  window.addEventListener(
    ...
    () => {
      // 节流：真实滚轮每秒可触发数十次事件
      const now = Date.now();
      if (now - lastScrollInject > 250) {
        lastScrollInject = now;
        maybeInject();
      }
    },
    { passive: true },
