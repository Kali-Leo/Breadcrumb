/**
 * Purpose: LLM term-marking — prompt assembly for the one-shot flash call that picks which
 * words in a just-finished answer would trip up this learner, plus the pure post-processing:
 * conditional density clipping (unlimited once the learner is well enough understood, else a
 * strict per-60-characters cap) and locating each surviving term's first occurrence in the
 * answer as a door-shaped patch.
 ...
 * evidence context. Both lists may be empty (a brand-new learner has neither). */
 ...
 * that every LLM-picked term ships uncapped. */
 ...
 * null — enrichment against known node labels is the caller's job). Terms are processed in
 * the given (obstruction-descending) order; a term that doesn't appear verbatim, or whose only
 * occurrence overlaps a reserved span or an already-placed term, is dropped — the result is
 * sorted back into reading order. */
