 ...
 * from a practice item — saved like any other conversation but hidden from the
 * sidebar's chat list, since it is a temporary offshoot of one practice leaf, not a standing
 * learning thread. 'teach' = a teach-back session; when its companion_id is set
 * the student side is played by the Shichimi companion card instead of the
...
export interface ConversationRow {
   ...
   * null for ordinary chat/practice threads that have no companion attached. */
  companion_id: string | null;
  /** System-maintained trail-card name, "first station -> last station" — null
...
export interface MessageRow {
   ...
   * "guided" — NULL otherwise. Kept as string here: core-db does not depend on
   * core-teaching. */
  teaching_mode: string | null;
  /** Tree parent: the message this one replies to (user msg -> current leaf,
