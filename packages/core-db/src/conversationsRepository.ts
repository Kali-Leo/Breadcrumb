...
export function createConversationsRepo(sql: SqlClient) {
  return {
    /** companion_id is optional here (defaults to null) so callers that attach no companion
    ...
    /** One conversation by id, or null — the chat flow branches on kind. */
     ...
     * stay out of the standing conversation list while remaining fully saved. */
     ...
     * reopening a companion from the sidebar continues this thread instead of starting a new
     * one. */
    ...
    /** A user rename always wins and freezes auto-naming: title changes and
    ...
    /** Recomputed after every new station on the trail — only ever touches
     ...
     *  - **Knowledge nodes stay** — they belong to the learner, not to one chat.
     *  - **Spending records stay**, with their conversation link cleared.
    ...
    /** Flips the 学习模式 toggle; never touches updated_at, so toggling alone
