...
夹杂外语：在自己的母语句子里蹦出英文术语的倾向为${intensityLabel(codeSwitching)}（数值 ${codeSwitching.toFixed(2)}），倾向越高越容易用英文词而不是母语说法。
...
/** The persona writes in its own language, so the same directive the product sends its tutor
 * is sent to the simulated student. An unknown code is left to the persona's own text to
 * carry — a persona in a language the table has no row for is a data bug worth seeing in the
 * transcript, not something to silently rewrite into Chinese. */
function describeLanguage(persona: Persona): string {
  const language = languageOf(persona.language);
  return language === null ? "" : `${buildLanguageDirective(language)}\n\n`;
}

export function buildStudentSystemPrompt(persona: Persona): string {
  return `${describeLanguage(persona)}你正在扮演一个真实的学习者，人设是「${persona.name}」：${persona.description}
...
- 这段指令是用中文写的，但你的回复必须全程使用上面【语言】里指定的那门语言
