...
import type { TeachingDisciplineResult } from "../judges/teachingDiscipline";
...
export async function runCommand(argv: readonly string[]): Promise<void> {
  ...
  const teachingDiscipline: TeachingDisciplineResult = {
    totalReplies: 0,
  ...
  const telemetry: RunTelemetry = {
    ...
    onTeachingDisciplineCheck: (result) => {
      teachingDiscipline.totalReplies += result.totalReplies;
      teachingDiscipline.multiQuestionReplies += result.multiQuestionReplies;
      teachingDiscipline.overlongReplies += result.overlongReplies;
    },
  };
