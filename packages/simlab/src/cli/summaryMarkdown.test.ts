...
describe("buildSummaryMarkdown", () => {
  ...
  it("never reports targetConceptsEcho, under that name or as a recall figure", () => {
    // It is an echo of the student prompt's own input, so it reads 100% on every run and
    // would be read as evidence the extraction works. It stays in metrics.json; putting
    // it back here needs a reason.
    const markdown = buildSummaryMarkdown(makeMetrics(), []);
