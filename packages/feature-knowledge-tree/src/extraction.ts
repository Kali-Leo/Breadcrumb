...
规则：
- 新学到的知识点：正常提取
- 这一轮实质性重温了已有树上的知识点：也列出，label 精确用树上原名；只顺带提及则不算
- 本轮概念与已有清单中某项是同一概念的不同说法时，直接使用清单原名
- label 用领域通用术语
- parentLabel 只能是已有知识树列表中某个节点的精确 label、本批次里另一个节点的 label，
  或 null（挂在顶层）；"顶层""根"之类的描述性文字不是节点名，不能填
