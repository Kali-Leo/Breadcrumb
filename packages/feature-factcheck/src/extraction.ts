...
/** One atomic claim, in the learner's language — it is rendered under the answer, so the
 * ceiling has to hold the same sentence in a script that is not written in hanzi. */
const CLAIM_BUDGET: LengthBudget = { cjkChars: 40, words: 15 };
  ...
  claims: z
    .array(
      z.object({
        ...
        text: z.string().min(1).max(maxCharsFor(CLAIM_BUDGET)),
...
{"claims":[{"text":"原子化声明（一句话、自包含、${lengthRule(CLAIM_BUDGET)}）","queries":["检索查询1","检索查询2（可选）"]}]}
规则：
...
- 为每条声明生成 2 个检索查询：用"关键词组合"而非完整句子（如：光速 数值 299792458），
  必须包含声明中最具区分度的数字、术语或人名；两条查询角度错开（如一条用声明本身的语言、一条用英文）
