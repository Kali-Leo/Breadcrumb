/**
 * Purpose: unit tests for the one reduction every mechanical grounding check rests on — what
 * counts as a verbatim quote, what normalising is allowed (whitespace, the passage block's own
 * fences, NFC) and what is not (any rewrite, however small).
 */
import { describe, expect, it } from "vitest";
import type { EvidenceItem } from "../evidence/provider";
import { foldEvidenceText, MIN_ANCHOR_CHARS, quoteIsGrounded } from "./anchorText";
...
describe("foldEvidenceText", () => {
  ...
  it("strips the fence literals the passage block wraps excerpts in", () => {
...
describe("quoteIsGrounded", () => {
  ...
  it("accepts a quote that differs from the source only in spacing", () => {
    ...
    // a strict rule refused correct findings over it — 9 lost against 5, with no false
    // support let through either way.
  ...
  it("rejects a quote that was reworded, however slightly", () => {
    // One character changed: this is the fabrication the check exists to catch.
