 ...
 * context novelty. Positive whenever reviewing helps; ~0 for just-reviewed words. */
function reviewScore(card: Card, now: Date, novelty: number): number {
  ...
  return (0.25 + urgency) * relativeGain * novelty;
}
...
export function scheduleReplacements(input: ScheduleInput): ScheduledReplacement[] {
  ...
  for (const candidate of input.candidates) {
    ...
    if (card !== undefined) {
      ...
      // New words score below any meaningfully due review (review first, then growth) and
      // among themselves by introduction rank: earlier queue position wins.
      scored.push({ ...candidate, kind: "new", score: 1 / (1000 + rank) });
    }
  }
  ...
  for (const item of scored) {
    ...
    if (item.kind === "review" && item.score <= 0) continue;
    chosen.push(item);
