...
## 体验优先于体积（2026-07-31 Leo 颁布）

- 项目最终体积（臃肿度）的优先级**低于**运行流畅度（硬件要求）：为了更好的运行体验，允许更大的依赖/资产/安装包
- 达标标准：在一台平均水平的 PC 上获得完美体验

## 开源优先铁律（2026-07-31 Leo 颁布）

- 凡不是 AI 最擅长的任务（无论多小），必须使用开源工具与方法完成；"最擅长"仅指业务粘合：数据接线、类型契约、测试、集成编排、文档
- 极端情况下确实找不到任何可用开源资源，须向 Leo 申请并获批后才允许自研
- 记忆宫殿模块内永久禁止 AI 自创的程序化图形画法；每个视觉元素必须可指认其复刻的官方开源出处（详见 memory 与 ADR-0006）
...
- **Leo 是唯一美术验收官**：任何素材必须出对照板并经 Leo 明确认可，才能标 approved、进 assets/、进 registry。AI 目检只是初筛（candidate）
- 形态/方向级的美术决策（元素长什么样、用什么视觉隐喻）必须先与 Leo 对齐，禁止以技术便利代替产品决策
- 纯白线稿合同：所有贴图为纯黑线稿（排线表达明暗），禁止灰色晕染；统一白底并程序化验收
- 贴图两轴世界观见 docs/vision/04：缩放等级（地理层/国度层/村落层）× 同层规模（簇大小）
